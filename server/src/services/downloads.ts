...
export const downloadProgress: Map<string, DownloadProgress> = new Map();
...
export async function downloadHuggingFaceModel(modelId: string): Promise<{ success: boolean; message: string; path?: string }>{
  const modelsDir = join(process.cwd(), 'models');
  const modelDir = join(modelsDir, `${modelId.replace('/', '--')}`);

  const progress: DownloadProgress = { model: modelId, status: 'downloading', progress: 0, message: 'Initializing download...', startTime: Date.now() };
...
export function cancelDownload(modelId: string) {
  const progress = downloadProgress.get(modelId);
  if (!progress) return { success: false, message: `No active download found for model ${modelId}` };
